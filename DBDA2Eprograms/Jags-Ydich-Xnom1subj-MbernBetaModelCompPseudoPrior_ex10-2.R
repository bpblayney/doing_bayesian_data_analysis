...
model {
  ...
  omega1[2] <- .272 # pseudo prior value
  kappa1[1] <- 202 # true prior value
  kappa1[2] <- 202 # pseudo prior value
  theta2 ~ dbeta( omega2[m]*(kappa2[m]-2)+1 , (1-omega2[m])*(kappa2[m]-2)+1 ) 
  omega2[1] <- .744 # pseudo prior value
  omega2[2] <- .75 # true prior value
  kappa2[1] <- 202 # pseudo prior value
  kappa2[2] <- 202 # true prior value
  m ~ dcat( mPriorProb[] )
  mPriorProb[1] <- .5
  mPriorProb[2] <- .5
}
