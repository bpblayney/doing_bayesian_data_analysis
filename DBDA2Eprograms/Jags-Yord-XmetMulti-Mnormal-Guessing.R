# Jags-Yord-XmetMulti-Mnormal-Guessing.R 
...
#  Rsq = zbeta %*% matrix( YcorX , ncol=1 )
  ...
  if ( pairsPlot ) {
    ...
    pairs( cbind( beta0 , beta , sigma , alpha )[plotIdx,] ,
           labels=c( "beta[0]" , 
  ...
  panelCount = decideOpenGraph( panelCount , saveName=paste0(saveName,"PostMarg") )
  histInfo = plotPost( alpha , cex.lab = 1.75 , showCurve=showCurve ,
                       xlab=bquote(alpha) , main=paste("Alpha") )
#   panelCount = decideOpenGraph( panelCount , saveName=paste0(saveName,"PostMarg") )
